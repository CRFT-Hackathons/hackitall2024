...
export default function Page() {
  ...
  const [volunteers] = useState<Contributor[]>([
    {
      id: 1,
      name: "Sarah Chen",
      contributions: 187,
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4 mb-4 md:mb-6">
            ...
            <div className="w-full sm:w-auto">
              ...
              <p className="text-sm md:text-base text-gray-500">
                Next achievement: 200 activities (13 to go!)
              </p>
              <div className="relative w-full sm:w-[60%] lg:w-[70%] xl:w-[60%] mt-2 pr-14">
                <div className="w-full h-1 sm:h-2 bg-purple-100 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-[#9333ea] transition-all duration-300 ease-in-out"
                    style={{ width: `${progress}%` }}
                  />
                </div>
                <span className="absolute right-0 top-1/2 -translate-y-1/2 text-xs md:text-sm text-gray-500 min-w-[3rem] text-right">
                  {Math.round(progress)}%
                </span>
              </div>
            </div>
          ...
          <Card>
            ...
            <CardContent className="p-2 md:p-4">
              <ScrollArea className="h-[300px] lg:h-[calc(100vh-300px)]">
                {volunteers.map((contributor) => (
                  <div
                    key={contributor.id}
                    className="flex items-center space-x-2 md:space-x-4 mb-3 p-2 rounded dark:hover:bg-white/5 hover:bg-black/5"
                  >
                    <div className="font-bold text-base md:text-lg w-4">
                      {contributor.rank}
                    </div>
